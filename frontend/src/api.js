...
export const api = {
  ...
  setLeadStatus: (leadId, status) => request(`/leads/${leadId}/status`, { method: "POST", body: { status } }),
  updateLead: (leadId, patch) => request(`/leads/${leadId}`, { method: "PATCH", body: patch }),
  getAllLeads: (status) => request(`/me/leads${status ? `?status=${encodeURIComponent(status)}` : ""}`),
